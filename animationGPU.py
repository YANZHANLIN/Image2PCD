...
def create_gpu_point_cloud_animation(x, y, z_frames, color_frames, output_filename='gpu_animation.mp4', 
                                   sample_rate=None, fps=30, rotation_degrees=0):
    ...
    if sample_rate is None:
        sample_rate = max(1, len(x) // 100000)  # Up to 200k points for GPU
    ...
    rotation_per_frame = rotation_degrees / num_frames
    ...
    for frame_idx in range(len(z_sample)):
        ...
        print(f"Total rotation so far: {total_rotation} degrees")
    ...
    print(f"Final total rotation: {total_rotation} degrees")
...
def create_interactive_gpu_viewer(x, y, z_frames, color_frames, sample_rate=None):
    ...
    print("- S: Save current camera parameters")
    print("- ESC: Exit")
    ...
    def save_camera_params(vis):
        ...
        print(f"Front: {params.extrinsic[:3, 2]}")
        print(f"Up: {params.extrinsic[:3, 1]}")
        print(f"Lookat: {params.extrinsic[:3, 3]}")
        print(f"Field of View: {params.intrinsic.get_fov()}")
        print(f"Camera Position: {params.extrinsic[:3, 3]}")
        print("\nTo use these parameters in create_gpu_point_cloud_animation, set:")
        print(f"ctr.set_front({params.extrinsic[:3, 2].tolist()})")
        print(f"ctr.set_up({params.extrinsic[:3, 1].tolist()})")
        print(f"ctr.set_lookat({params.extrinsic[:3, 3].tolist()})")
        return False
    ...
    vis.register_key_callback(ord('S'), save_camera_params)  # S key
...
if __name__ == '__main__':
    ...
    try:
        ...
        create_gpu_point_cloud_animation(x, y, z_frames, color_frames, 
                                       'gpu_point_cloud_animation.mp4', fps=5)
